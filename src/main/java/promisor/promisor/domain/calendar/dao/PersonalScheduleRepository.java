...
public interface PersonalScheduleRepository extends JpaRepository<PersonalSchedule, Long>{
    ...
    @Query(value = "select * from personal_shedule ps join personal_calendar pc on ps.personal_calendar_id = pc.id\n" +
    ...
    @Query("select ps from PersonalSchedule ps join ps.personalCalendar pc where pc = :pc")
    List<PersonalSchedule> findAllByPersonalCalendar(@Param("pc") PersonalCalendar pc);
}
